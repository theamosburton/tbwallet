...
Usage: tulobyte <flags> or <SUBCOMMANDS> <sub-flags>
...
SUBCOMMANDS:
    ...
    delete                               Delete a specific wallet from the system.
                                         Note: Deleted wallets cannot be recovered if lost.
    disp-address                         Display your wallet address.
    disp-pubkey                          Display your wallet's public key.
...
func PrintConfigHelp() {
	helpText := `
Usage: tulobyte config <tags> <values>
...
Values:
    -rpc mainnet               Switch the network to mainnet.
    -rpc testnet               Switch the network to testnet.
    -rpc -d                    Display the current network configuration.
...
func PrintTxnHelp() {
	helpText := `
Usage: tulobyte txn <RECIPIENT_ADDRESS> <AMOUNT> <DATA>
...
func PrintTxnHelpShort(argsReq string) {
 ...
 or type  "tulobyte txn -h" for help and more commands
    `)
}
